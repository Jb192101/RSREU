...
SELECT 
    id,
    название_проекта,
    бюджет::text as бюджет
FROM projects
WHERE бюджет IS NOT NULL
...
SELECT 
    id,
    название_проекта,
    'неизвестен' as бюджет
FROM projects
WHERE бюджет IS NULL;
...
SELECT 
    ...
    a.id_проекта,
    a.отработано_часов,
    a.ставка_в_час as исходная_ставка,
    CASE 
        WHEN total_hours < 1000 THEN a.ставка_в_час * 0.9    -- -10%
        WHEN total_hours BETWEEN 1000 AND 5000 THEN a.ставка_в_час -- 0% (без изменений)
        WHEN total_hours > 5000 THEN a.ставка_в_час * 1.2    -- +20%
        ELSE a.ставка_в_час                                   -- на случай NULL
    END as новая_ставка,
    a.дата_назначения
FROM assingments a
LEFT JOIN (
    SELECT 
        id_сотрудника,
        COALESCE(SUM(отработано_часов), 0) as total_hours
    FROM assingments
    GROUP BY id_сотрудника
) as employee_hours ON a.id_сотрудника = employee_hours.id_сотрудника;
